...
begin
    ...
    contour!(xRange₁, xRange₂, fSamples, label = L"$f_{\vec{\mathbf{X}}}(\vec{\mathbf{x}})$",
        levels = 10,
        color = (:steelblue, 0.5))
    scatter!(Samples[:, 1], Samples[:, 2], label = L"$f_{\vec{\mathbf{X}}}(\vec{\mathbf{x}})$",
        color = (:steelblue, 0.5),
        markersize = 6)
    
    Samples = analyze(Problem, IS(q, 500)).Samples
    contour!(xRange₁, xRange₂, qSamples, label = L"$q(\vec{\mathbf{x}})$",
        levels = 10,
        color = (:crimson, 0.5))
    scatter!(Samples[:, 1], Samples[:, 2], label = L"$q(\vec{\mathbf{x}})$",
        color = (:crimson, 0.5),
        markersize = 6)
